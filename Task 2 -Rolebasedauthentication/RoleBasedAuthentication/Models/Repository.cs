...
namespace RoleBasedAuthentication.Models{
    ...
    public class Repository{
        public static bool login(User user){
            ...
            connection.Open();
            SqlCommand sqlcommand = new SqlCommand($"select count(*) from Table_1 where Username=@username and Password=@password",connection);
         ...
         public static string GetRole(User user){
            ...
            connection.Open();
            SqlCommand sqlcommand = new SqlCommand($"select Role from Table_1 where Username=@username",connection);
            sqlcommand.Parameters.Add("@username",sqlDbType:System.Data.SqlDbType.VarChar).Value = user.Username;
